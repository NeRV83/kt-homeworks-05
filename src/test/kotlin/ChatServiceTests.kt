...
class ChatServiceTest {
    ...
    @Test
    fun sendMessageShouldCreateNewChatWhenSendingFirstMessageToUser() {
    ...
    fun getLastMessagesShouldReturnCorrectLastMessagesForEachChat() {
        ...
        println("Actual lastMessages: $lastMessages")
        println("Expected messages: [Второе сообщение пользователю 2, Единственное сообщение пользователю 3]")

        assertEquals(2, lastMessages.size)

        // Проверяем наличие обоих сообщений
        val containsMessage1 = lastMessages.any { it == "Второе сообщение пользователю 2" }
        val containsMessage2 = lastMessages.any { it == "Единственное сообщение пользователю 3" }

        assertTrue("Message 'Второе сообщение пользователю 2' not found in $lastMessages", containsMessage1)
        assertTrue("Message 'Единственное сообщение пользователю 3' not found in $lastMessages", containsMessage2)
    }
